...
impl Default for Ads1115Conf {
    fn default() -> Ads1115Conf {
        Ads1115Conf {
...
impl Ads1115 {
    pub fn new(aqc : Ads1115Conf) -> Result<Ads1115, LinuxI2CError> {
    ...
    pub fn get_raw_val(&self, ch : &Channel) -> Result<u16, LinuxI2CError> {
        let mut dev = LinuxI2CDevice::new("/dev/i2c-1", ADC_I2C_ADDR)?;
        let mut ads_cfg = self.ads_cfg;
        ads_cfg.ch = *ch;
        let conf = ads_cfg.as_bit_msk() | misc_cfg::FIRE_SINGLE_CONV;
        dev.smbus_write_i2c_block_data(regs::CONF, &[(conf >> 8) as u8, conf as u8])?;
        loop { 
    ...
    pub fn get_voltage(&self, ch : &Channel) -> Result<f32, LinuxI2CError> {
        let raw = self.get_raw_val(ch)?;
        let voltage = match self.ads_cfg.fsr {
...
pub struct Ads1115Channel<'a> {
    ch : Channel,
    adc : &'a Ads1115,
}

impl Ads1115Channel<'_> {
    pub fn new(adc : &Ads1115, ch : Channel) -> Ads1115Channel {
        Ads1115Channel {
            ch,
            adc,
        }
    ...
    pub fn get_voltage(&self) -> Result<f32, LinuxI2CError> {
        self.adc.get_voltage(&self.ch)
    }
}

impl AdcCh for Ads1115Channel<'_> {
    fn get_voltage(&self) -> Result<f32, AdcErr> {
        let ch = Channel::ChRelAn0;
        match self.get_voltage() {
            Ok(sig) => Ok(sig),
            Err(_) => Err(AdcErr::new("I2C error")),
        }
    }
